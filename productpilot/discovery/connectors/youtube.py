"""YouTube comment connector using local seed data."""
...
class YouTubeConnector(SourceConnector):
    source_name = SourceName.YOUTUBE.value
    ...
    def fetch(self) -> list[NormalizedRecord]:
        ...
        for item in payload:
            ...
            content_hash = NormalizedRecord.build_content_hash(SourceName.YOUTUBE, item["text"], item["url"])
            records.append(
                NormalizedRecord(
