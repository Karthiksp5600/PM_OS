"""Play Store connector vertical slice using local seed data."""
...
class PlayStoreConnector(SourceConnector):
    source_name = SourceName.PLAY_STORE.value
    ...
    def fetch(self) -> list[NormalizedRecord]:
        ...
        for item in payload:
            ...
            content_hash = NormalizedRecord.build_content_hash(SourceName.PLAY_STORE, item["text"], item["url"])
            records.append(
                NormalizedRecord(
