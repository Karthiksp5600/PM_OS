"""Twitter/X search result connector using local seed data."""
...
class TwitterXConnector(SourceConnector):
    source_name = SourceName.TWITTER_X.value
    ...
    def fetch(self) -> list[NormalizedRecord]:
        ...
        for item in payload:
            ...
            content_hash = NormalizedRecord.build_content_hash(SourceName.TWITTER_X, item["text"], item["url"])
            records.append(
                NormalizedRecord(
                    source=SourceName.TWITTER_X,
                    url=item["url"],
